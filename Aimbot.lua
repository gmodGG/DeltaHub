...
safeCall(function()
    ...
    FPSsec:Toggle({ Title = "Show FPS Counter", Desc = "عداد FPS حي أعلى يسار", Value = Config.Performance.ShowFPS, Callback = function(v) Config.Performance.ShowFPS = v end })
    FPSsec:Slider({ Title = "FPS Cap", Desc = "يقفل الإطارات = بطارية أطول", Step = 5, Value = { Min = 20, Max = 144, Default = Config.Performance.FPSCap }, Callback = function(v) Config.Performance.FPSCap = v; SetFPS(v) end })
    FPSsec:Button({ Title = "Unlimited FPS", Icon = "zap", Callback = function() SetFPS(9999) end })
    ...
    QualSec:Button({ Title = "Auto (Recommended)", Icon = "sparkles", Color = Color3.fromRGB(94, 255, 170), Desc = "Medium + قفل 60 = توازن للجوال", Callback = function() ApplyQuality("Medium"); SetFPS(60); Config.Performance.FPSCap = 60 end })
    QualSec:Toggle({ Title = "Disable Post-FX", Desc = "يطفئ Bloom/Blur (يرفع الأداء)", Value = Config.Performance.PostFXOff, Callback = function(v) Config.Performance.PostFXOff = v; SetPostFX(v) end })
    QualSec:Toggle({ Title = "Disable Particles & Effects", Desc = "يطفئ Particles/Beams/Trails", Value = Config.Performance.EffectsOff, Callback = function(v) Config.Performance.EffectsOff = v; SetEffects(v) end })
    local LightSec = PerfTab:Section({ Title = "Lighting Tweaks", Box = true, BoxBorder = true, Opened = false })
    LightSec:Toggle({ Title = "Shadows", Value = true, Callback = function(v) SetShadows(v) end })
    LightSec:Slider({ Title = "Fog Distance", Desc = "أقل = أداء أعلى", Step = 100, Value = { Min = 200, Max = 50000, Default = 12000 }, Callback = function(v) SetFog(v) end })
    LightSec:Slider({ Title = "Brightness", Step = 0.1, Value = { Min = 0, Max = 3, Default = 1.5 }, Callback = function(v) SetBright(v) end })
    local ResetSec = PerfTab:Section({ Title = "Reset", Box = true, BoxBorder = true, Opened = true })
    ResetSec:Button({ Title = "Restore Original", Icon = "rotate-ccw", Color = Color3.fromRGB(255, 180, 80), Desc = "يرجّع إعدادات اللعبة لأصلها", Callback = function() RestoreAll() end })
    ...
    AimTab:Select()
    WindUI:Notify({ Title = "Delta Hub", Content = "نسخة Performance النظيفة جاهزة ⚡", Icon = "check", Duration = 5 })
end)
