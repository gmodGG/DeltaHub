...
║  ✅ Lag Fix + 🆘 Performance Tab كامل          ║
║  FPS Cap • Graphics • Lighting • FPS Counter   ║
╚═══════════════════════════════════════════════╝
]]
...
local Config = {
    ...
    -- 🆘 إعدادات الأداء الافتراضية (تتطبق من القائمة)
    Performance = {
        FPSCap = 60,
        ShowFPS = false,
        PostFXOff = false,
...
-- ═══════════════════════════════════════════
-- 🆘 PERFORMANCE ENGINE (جديد بالكامل)
-- ═══════════════════════════════════════════
local Perf = {}
...
Perf.SavedEffects = {}
Perf.EffectConn = nil
Perf.FPS = { frames = 0, last = tick(), value = 0, draw = nil }

-- ═══ قفل الإطارات (آمن — pcall لأن مو كل executor يدعمها) ═══
local function SetFPS(n)
    if type(setfpscap) == "function" then
        pcall(setfpscap, math.floor(n))
    end
end
-- حفظ القفل الأصلي مرة وحدة (لو متاح)
safeCall(function()
...
-- ═══ حفظ إعدادات الإضاءة الأصلية (للـ Restore) ═══
...
-- ═══ مستويات الجودة ═══
local PRESETS = {
    ["Ultra"]   = { GlobalShadows = true,  ShadowSoftness = 1,   Brightness = 2,   FogEnd = 100000, Tech = "ShadowMap", PostFX = true  },
    ["High"]    = { GlobalShadows = true,  ShadowSoftness = 0.5, Brightness = 2,   FogEnd = 40000,  Tech = "ShadowMap", PostFX = true  },
    ["Medium"]  = { GlobalShadows = false, ShadowSoftness = 0,   Brightness = 1.5, FogEnd = 12000,  Tech = "ShadowMap", PostFX = false },
    ["Low"]     = { GlobalShadows = false, ShadowSoftness = 0,   Brightness = 1,   FogEnd = 3000,   Tech = "Voxel",     PostFX = false },
    ["Potato"]  = { GlobalShadows = false, ShadowSoftness = 0,   Brightness = 1,   FogEnd = 800,    Tech = "Legacy",    PostFX = false },
}

local function SetPostFX(off)
    local classes = { "BloomEffect","BlurEffect","ColorCorrectionEffect","SunRaysEffect","DepthOfFieldEffect","Atmosphere" }
    if off then
        for _, obj in ipairs(Lighting:GetDescendants()) do
            for _, cls in ipairs(classes) do
                if obj:IsA(cls) then
                    Perf.SavedPostFX[obj] = obj.Enabled
                    pcall(function() obj.Enabled = false end)
                end
            end
        end
    else
        for obj, en in pairs(Perf.SavedPostFX) do
            pcall(function() obj.Enabled = en end)
        end
...
local function ApplyQuality(name)
    ...
    SetPostFX(p.PostFX == false)
    Config.Performance.PostFXOff = (p.PostFX == false)
end

-- ═══ تعطيل Particles / Beams / Trails (أكبر قفزة أداء) ═══
...
local function KillEffect(obj)
    for _, cls in ipairs(EFFECT_CLASSES) do
        if obj:IsA(cls) then
            Perf.SavedEffects[obj] = obj.Enabled
            pcall(function() obj.Enabled = false end)
        end
    end
end
...
-- ═══ تحكم فردي ═══
local function SetShadows(on) CaptureLighting(); pcall(function() Lighting.GlobalShadows = on end) end
local function SetFog(dist)    CaptureLighting(); pcall(function() Lighting.FogEnd = dist; Lighting.FogStart = 0 end) end
local function SetBright(v)    CaptureLighting(); pcall(function() Lighting.Brightness = v end) end

-- ═══ Restore كل شيء لأصله ═══
...
-- تطبيق القفل الافتراضي عند التشغيل
...
function ESP:Create(player)
    ...
    d.Name.Size = Config.ESP.NameSize; d.Name.Center = true; d.Name.Outline = true; d.Name.OutlineColor = OC; d.Name.Color = Color3.fromRGB(255, 255, 255); d.Name.Font = Drawing.Fonts.Plex; d.Name.Visible = false
    d.Dist.Size = Config.ESP.DistSize; d.Dist.Center = true; d.Dist.Outline = true; d.Dist.OutlineColor = OC; d.Dist.Color = Color3.fromRGB(185, 190, 205); d.Dist.Font = Drawing.Fonts.Plex; d.Dist.Visible = false
...
function ESP:DrawPlayer(c, cam, centerX, bottomY, viewport)
    ...
    if Config.ESP.BoxEnabled then
        d.BoxOutline.Visible = true; d.BoxOutline.Position = Vector2.new(boxX, boxY); d.BoxOutline.Size = Vector2.new(boxW, boxH)
        d.Box.Visible = true; d.Box.Color = color; d.Box.Position = Vector2.new(boxX, boxY); d.Box.Size = Vector2.new(boxW, boxH)
    ...
    if Config.ESP.Tracers then
        ...
        if Config.ESP.TracerOutline then
            d.TracerOutline.Visible = true; d.TracerOutline.From = from; d.TracerOutline.To = to
    ...
    if Config.ESP.HeadDot then
        ...
        d.HeadDot.Visible = true; d.HeadDot.Position = Vector2.new(headScreen.X, headScreen.Y)
        d.HeadDot.Radius = ClampNumber(boxW * 0.17, 2.5, 8); d.HeadDot.Color = color
    else d.HeadDot.Visible = false end

    if Config.ESP.HealthBar then
        local barW = Config.ESP.HealthBarWidth
        local barX = boxX - barW - 5
        d.HP_Outline.Visible = true; d.HP_Outline.Position = Vector2.new(barX - 1, boxY - 1); d.HP_Outline.Size = Vector2.new(barW + 2, boxH + 2)
        d.HP_BG.Visible = true; d.HP_BG.Position = Vector2.new(barX, boxY); d.HP_BG.Size = Vector2.new(barW, boxH)
        d.HP.Visible = true; d.HP.Position = Vector2.new(barX, boxY + boxH * (1 - hpRatio))
        d.HP.Size = Vector2.new(barW, math.max(boxH * hpRatio, hpRatio > 0 and 1 or 0)); d.HP.Color = hpColor
    else d.HP_Outline.Visible = false; d.HP_BG.Visible = false; d.HP.Visible = false end

    if Config.ESP.Names then
        d.Name.Visible = true; d.Name.Text = plr.DisplayName or plr.Name; d.Name.Position = Vector2.new(rootScreen.X, boxY - 16)
    else d.Name.Visible = false end

    if Config.ESP.Distance then
        d.Dist.Visible = true; d.Dist.Text = string.format("%.0f m", dist); d.Dist.Position = Vector2.new(rootScreen.X, yBottom + 5)
...
Players.PlayerRemoving:Connect(function(plr) safeCall(function() ESP:Remove(plr) end) end)
...
-- ═══════════════════════════════════════════
-- GOD AIMBOT (Throttled)
-- ═══════════════════════════════════════════
pcall(function() RunService:UnbindFromRenderStep("DeltaAimbot") end)
local Aimbot = { Locked = nil, LastSwitch = 0, LastShot = 0, LastRetarget = 0, LockVisTime = 0, LockVisResult = true, LockVisPart = nil }
local AimbotConnections = {}
pcall(function()
    local old = getgenv().DeltaAimbotConnections
    if old then for _, c in ipairs(old) do pcall(function() c:Disconnect() end) end end
    getgenv().DeltaAimbotConnections = AimbotConnections
end)
local function ConnectAimbot(signal, fn) local c = signal:Connect(fn); table.insert(AimbotConnections, c); return c end
local function AimSafe(fn, ...) local ok, err = pcall(fn, ...); if not ok then warn("[Delta Aimbot]", err) end; return ok, err end
local function IsFiniteNumber(n) return typeof(n) == "number" and n == n and n ~= math.huge and n ~= -math.huge end
local function AimClamp(n, min, max) if not IsFiniteNumber(n) then return min end; if n < min then return min end; if n > max then return max end; return n end
local function IsFiniteVector(v) return typeof(v) == "Vector3" and IsFiniteNumber(v.X) and IsFiniteNumber(v.Y) and IsFiniteNumber(v.Z) end
...
local function IsEnemy(plr)
    ...
    local meTeam = LocalPlayer.Team; local plrTeam = plr.Team
...
local function MaybeRefreshRayIgnore()
    ...
    if cam ~= lastCamera or char ~= lastCharacter then
        Camera = cam; lastCamera = cam; lastCharacter = char; RefreshRayIgnore()
    end
end
...
local function GetBestAimPart(plr)
    ...
    local function Consider(name, weight)
        ...
        if part and IsFiniteVector(part.Position) and (not wallCheck or IsPartVisible(part)) then
            ...
            if IsFiniteNumber(score) and score < bestScore then bestScore = score; bestPart = part end
        end
    end
    Consider(C.AimPart, 0.55); Consider("Head", 0.60); Consider("HumanoidRootPart", 0.78)
    return bestPart
end
...
function Aimbot:GetTarget()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and IsEnemy(plr) and IsPlayerAlive(plr) then
            local char = plr.Character
            local part = GetPartByName(char, C.AimPart) or GetPartByName(char, "Head") or GetPartByName(char, "HumanoidRootPart")
            if part then
...
function Aimbot:IsLockedValid()
    ...
    if typeof(t.Part) ~= "Instance" or not t.Part.Parent then
        ...
        t.Part = newPart; self.LockVisPart = newPart; self.LockVisResult = true; self.LockVisTime = tick()
    end
    if C.WallCheck ~= false then
        local now = tick()
        if self.LockVisPart ~= t.Part or now - self.LockVisTime > 0.15 then
            self.LockVisTime = now; self.LockVisPart = t.Part
...
function Aimbot:ForceSwitch() self.Locked = nil; self.LastSwitch = tick() end
local function TrySwitchLock()
    ...
    if C.LockSwitchKey then
        ...
        if ok and down and tick() - Aimbot.LastSwitch > (C.SwitchCooldown or 0.3) then Aimbot:ForceSwitch() end
    end
end
...
-- ═══════════════════════════════════════════
-- تشغيل (3 binds: ESP + Aimbot + Perf/FPS)
-- ═══════════════════════════════════════════
...
-- 🆘 bind خفيف لعداد FPS فقط (بدون raycast/loop)
RunService:BindToRenderStep("DeltaPerf", 100, function(dt)
    ...
    if now - Perf.FPS.last >= 0.5 then
        ...
        if Perf.FPS.draw then
            if Config.Performance.ShowFPS then
                Perf.FPS.draw.Text = "FPS: " .. Perf.FPS.value
                Perf.FPS.draw.Color = Perf.FPS.value >= 50 and Color3.fromRGB(94, 255, 170)
                    or Perf.FPS.value >= 30 and Color3.fromRGB(255, 210, 90)
                    or Color3.fromRGB(255, 94, 98)
                Perf.FPS.draw.Visible = true
            else
                Perf.FPS.draw.Visible = false
            end
        end
...
print("[Delta] Mobile Pro + Performance Tab loaded.")
...
safeCall(function()
    ...
    local Window = WindUI:CreateWindow({ Title = "Delta Hub", Author = "Mobile Pro", Icon = "crosshair", Theme = "Dark" })
    ...
    AimMain:Dropdown({ Title = "Aim Part", Values = { "Head", "HumanoidRootPart", "UpperTorso", "Torso" }, Value = Config.Aimbot.AimPart, Callback = function(v) Config.Aimbot.AimPart = v end })
    ...
    AimPred:Slider({ Title = "Prediction Factor", Step = 0.05, Value = { Min = 0, Max = 1, Default = Config.Aimbot.PredictionFactor }, Callback = function(v) Config.Aimbot.PredictionFactor = v end })
    local AimBeh = AimTab:Section({ Title = "Behavior", Box = true, BoxBorder = true, Opened = true })
    AimBeh:Slider({ Title = "Smoothness", Step = 0.05, Value = { Min = 0.05, Max = 1, Default = Config.Aimbot.Smoothness }, Callback = function(v) Config.Aimbot.Smoothness = v end })
    ...
    AimBeh:Slider({ Title = "Target Refresh (s)", Desc = "أكبر = سلاسة أكثر", Step = 0.01, Value = { Min = 0.05, Max = 0.4, Default = Config.Aimbot.RetargetInterval }, Callback = function(v) Config.Aimbot.RetargetInterval = v end })
    AimBeh:Keybind({ Title = "Switch Lock Key", Value = "LeftShift", Callback = function(keyName) if keyName ~= "MouseLeft" and keyName ~= "MouseRight" and Enum.KeyCode[keyName] then Config.Aimbot.LockSwitchKey = Enum.KeyCode[keyName] end end })
    ...
    ESPMain:Toggle({ Title = "Wall Check Colors", Desc = "أخضر = ظاهر | أحمر = خلف جدار", Value = Config.ESP.WallCheck, Callback = function(v) Config.ESP.WallCheck = v end })
    ...
    ESPPerf:Slider({ Title = "Wall Check Speed", Desc = "1 = أخف، 4 = أدق", Step = 1, Value = { Min = 1, Max = 4, Default = Config.ESP.VisPerFrame }, Callback = function(v) Config.ESP.VisPerFrame = v end })
    ESPPerf:Slider({ Title = "ESP FPS", Step = 5, Value = { Min = 15, Max = 60, Default = Config.ESP.MobileFPS }, Callback = function(v) Config.ESP.MobileFPS = v end })
    ...
    -- ═══════════ 🆘 PERFORMANCE (القسم الجديد الكامل) ═══════════
    ...
    FPSsec:Toggle({
        Title = "Show FPS Counter", Desc = "عداد FPS حي أعلى يسار (أخضر/أصفر/أحمر)",
        Value = Config.Performance.ShowFPS,
        Callback = function(v) Config.Performance.ShowFPS = v end,
    })
    FPSsec:Slider({
        Title = "FPS Cap", Desc = "يقفل الإطارات = بطارية أطول + استقرار",
        Step = 5, Value = { Min = 20, Max = 144, Default = Config.Performance.FPSCap },
        Callback = function(v) Config.Performance.FPSCap = v; SetFPS(v) end,
    })
    FPSsec:Button({
        Title = "Unlimited FPS", Icon = "zap",
        Callback = function() SetFPS(9999) end,
    })
    ...
    QualSec:Button({
        Title = "Auto (Recommended)", Icon = "sparkles", Color = Color3.fromRGB(94, 255, 170),
        Desc = "Medium + قفل 60 + ضباب = توازن للجوال",
        Callback = function()
            ApplyQuality("Medium")
            SetFPS(60); Config.Performance.FPSCap = 60
        end,
    })
    QualSec:Toggle({
        Title = "Disable Post-FX", Desc = "يطفئ Bloom/Blur/ColorCorrection (يرفع الأداء)",
        Value = Config.Performance.PostFXOff,
        Callback = function(v) Config.Performance.PostFXOff = v; SetPostFX(v) end,
    })
    QualSec:Toggle({
        Title = "Disable Particles & Effects", Desc = "يطفئ Particles/Beams/Trails (قفزة أداء ضخمة)",
        Value = Config.Performance.EffectsOff,
        Callback = function(v) Config.Performance.EffectsOff = v; SetEffects(v) end,
    })

    local LightSec = PerfTab:Section({ Title = "Lighting Tweaks 💡", Box = true, BoxBorder = true, Opened = false })
    LightSec:Toggle({
        Title = "Shadows", Value = true,
        Callback = function(v) SetShadows(v) end,
    })
    LightSec:Slider({
        Title = "Fog Distance", Desc = "أقل = أداء أعلى (يخفي البعيد)",
        Step = 100, Value = { Min = 200, Max = 50000, Default = 12000 },
        Callback = function(v) SetFog(v) end,
    })
    LightSec:Slider({
        Title = "Brightness", Step = 0.1, Value = { Min = 0, Max = 3, Default = 1.5 },
        Callback = function(v) SetBright(v) end,
    })

    local ResetSec = PerfTab:Section({ Title = "Reset ♻️", Box = true, BoxBorder = true, Opened = true })
    ResetSec:Button({
        Title = "Restore Original", Icon = "rotate-ccw", Color = Color3.fromRGB(255, 180, 80),
        Desc = "يرجّع الإضاءة/الإطارات/المؤثرات لأصل اللعبة",
        Callback = function() RestoreAll() end,
    })
    ...
    SetSec:Button({
        ...
        Callback = function()
            ...
            safeCall(function() if Perf.FPS.draw then Perf.FPS.draw:Remove() end end)
            safeCall(function() Window:Destroy() end)
        end,
    })

    AimTab:Select()
    WindUI:Notify({ Title = "Delta Hub", Content = "نسخة Performance — قسم الأداء جاهز ⚡", Icon = "check", Duration = 5 })
end)
