--[[
╔═══════════════════════════════════════════════╗
║  DELTA HUB — Mobile Pro                        ║
║  Executor: Delta (Android / iOS)               ║
║  Features: Aimbot • ESP • Protection • Menu    ║
║  Status: ✅ Stable — Mobile Optimized          ║
═══════════════════════════════════════════════╝
• ESP: 🟢 أخضر = ظاهر بوجهك | 🔴 أحمر = خلف الجدار
• قائمة WindUI (افتحها من شريط Delta Hub أعلى يمين)
• بدون زر عائم — WindUI يوفر زره الخاص
]]
...
-- ═══════════════════════════════════════════
-- PROTECTION (قابل للتحكم من القائمة)
-- ═══════════════════════════════════════════
...
safeCall(function()
    LocalPlayer.CharacterAdded:Connect(function(char)
        safeCall(function()
            if Config.Protection.AntiFreeze then
                ApplyAntiFreeze(char)
            end
        end)
    end)
...
function ESP:IsPartVisible(part)
    ...
    if typeof(dist) ~= "number" or dist ~= dist or dist == math.huge then
        return false
    end
    ...
    if LocalPlayer.Character then
        ESPRayIgnore[1] = LocalPlayer.Character
    end
...
function ESP:CheckVisible(plr, head, root)
    local now = tick()
    local last = ESP.VisCacheTime[plr]
    if last and now - last < Config.ESP.WallCheckInterval then
        return ESP.VisCache[plr]
    end
    ESP.VisCacheTime[plr] = now
    local visible = self:IsPartVisible(head) or self:IsPartVisible(root)
    ESP.VisCache[plr] = visible
    return visible
end
...
function ESP:DrawPlayer(c, cam, centerX, bottomY, viewport)
    ...
    if Config.ESP.WallCheck then
        visible = self:CheckVisible(plr, head, root)
    end
    ...
    if Config.ESP.CornersEnabled then
        ...
        d.Corners[1].From = Vector2.new(boxX, boxY + cs)
        d.Corners[1].To   = Vector2.new(boxX, boxY)
        d.Corners[2].From = Vector2.new(boxX, boxY)
        d.Corners[2].To   = Vector2.new(boxX + cs, boxY)
        d.Corners[3].From = Vector2.new(boxX + boxW - cs, boxY)
        d.Corners[3].To   = Vector2.new(boxX + boxW, boxY)
        d.Corners[4].From = Vector2.new(boxX + boxW, boxY)
        d.Corners[4].To   = Vector2.new(boxX + boxW, boxY + cs)
        d.Corners[5].From = Vector2.new(boxX, boxY + boxH - cs)
        d.Corners[5].To   = Vector2.new(boxX, boxY + boxH)
        d.Corners[6].From = Vector2.new(boxX, boxY + boxH)
        d.Corners[6].To   = Vector2.new(boxX + cs, boxY + boxH)
        d.Corners[7].From = Vector2.new(boxX + boxW - cs, boxY + boxH)
        d.Corners[7].To   = Vector2.new(boxX + boxW, boxY + boxH)
        d.Corners[8].From = Vector2.new(boxX + boxW, boxY + boxH - cs)
...
LocalPlayer.CharacterAdded:Connect(function()
    safeCall(function()
        ...
        table.clear(ESP.VisCacheTime)
    end)
end)

-- ═══════════════════════════════════════════
-- GOD AIMBOT + TARGET LOCK
-- ═══════════════════════════════════════════
pcall(function() RunService:UnbindFromRenderStep("DeltaAimbot") end)

local Aimbot = { Locked = nil, LastSwitch = 0, LastShot = 0 }
...
local function IsEnemy(plr)
    ...
    if not C.TeamCheck then
        return true
    end
    local meTeam = LocalPlayer.Team
    local plrTeam = plr.Team
    if meTeam == nil or plrTeam == nil then
        return true
    end
...
local function RefreshRayIgnore()
    ...
    if lastCamera then
        table.insert(ignore, lastCamera)
    end
...
local function IsPartVisible(part)
    ...
    if not IsFiniteVector(origin) or not IsFiniteVector(targetPos) then
        return false
    end
    local dir = targetPos - origin
    local dist = dir.Magnitude
    if not IsFiniteNumber(dist) then
        return false
    end
    if dist < 0.05 then
        return true
...
local function GetBestAimPart(plr)
    ...
    local preferred = {C.AimPart, "Head", "HumanoidRootPart", "UpperTorso", "Torso"}

    if not dynamicBone then
        for _, name in ipairs(preferred) do
            local part = GetPartByName(char, name)
            if part and (not wallCheck or IsPartVisible(part)) then
                return part
            end
        end
        return nil
    ...
    local function Consider(name, weight)
        if not name or checked[name] then
            return
        end
        checked[name] = true
        local part = GetPartByName(char, name)
    ...
    Consider("UpperTorso", 0.80)
    Consider("Torso", 0.82)
    Consider("LowerTorso", 0.90)
...
local function GetTargetScore(plr, part)
    local C = Config.Aimbot
    if typeof(part) ~= "Instance" or not part.Parent or not Camera then
        return nil
    end
    local partPos = part.Position
    if not IsFiniteVector(partPos) then
        return nil
    end
    ...
    if not IsFiniteNumber(screenDist) then
        return nil
    end
    local fov = C.FOV or 0
    if fov > 0 and screenDist > fov then
        return nil
    end
...
function Aimbot:GetTarget()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and IsEnemy(plr) and IsPlayerAlive(plr) then
            ...
            if part then
                ...
                if score then
                    table.insert(candidates, {Player = plr, Score = score})
                end
            end
        end
    end
    ...
    for i = 1, math.min(maxChecks, #candidates) do
        ...
        if part then
            ...
            if score then
                return {Player = cand.Player, Part = part, Score = score}
            end
        end
    end
    return nil
...
function Aimbot:IsLockedValid()
    local C = Config.Aimbot
    local t = self.Locked
    if not t or not t.Player or not t.Part then
        return false
    end
    if t.Player == LocalPlayer then
        return false
    end
    if not IsEnemy(t.Player) then
        return false
    end
    if not IsPlayerAlive(t.Player) then
        return false
    end
    ...
    if C.WallCheck ~= false and not IsPartVisible(t.Part) then
        local newPart = GetBestAimPart(t.Player)
        if not newPart then
            return false
        end
    ...
    if fov > 0 then
        ...
        if not screenDist then
            return false
        end
        local lockFov = fov * (C.LockFOVScale or 1.35)
        if screenDist > lockFov then
            return false
        end
    end
    return true
...
local function TrySwitchLock()
    ...
    if C.LockSwitchKey then
        local ok, down = pcall(function()
            return UserInputService:IsKeyDown(C.LockSwitchKey)
        end)
...
local function PredictPosition(part, basePos)
    local C = Config.Aimbot
    if not C.Prediction then
        return basePos
    end
    if typeof(part) ~= "Instance" or not part:IsA("BasePart") or not Camera then
        return basePos
    end
    if not IsFiniteVector(basePos) then
        return basePos
    end
    local vel = part.AssemblyLinearVelocity or part.Velocity or Vector3.zero
    if not IsFiniteVector(vel) then
        vel = Vector3.zero
    end
    local origin = Camera.CFrame.Position
    ...
    if speed > 0 then
        ...
        travelTime = dist / 1200
    end
    travelTime = travelTime * (C.PredictionFactor or 0.2)
    if not IsFiniteNumber(travelTime) then
        travelTime = 0
    end
...
local function AimAtPosition(targetPos, dt)
    local C = Config.Aimbot
    if not Camera or not IsFiniteVector(targetPos) then
        return
    end
    local origin = Camera.CFrame.Position
    ...
    if (targetPos - origin).Magnitude < 0.01 then
        return
    end
...
local function TryAutoShoot()
    local C = Config.Aimbot
    if not C.AutoShoot then
        return
    end
    local delay = C.AutoShootDelay or 0.09
    if tick() - Aimbot.LastShot < delay then
        return
    end
...
function Aimbot:Update(dt)
    ...
    TrySwitchLock()
    if C.LockTarget and self.Locked and self:IsLockedValid() then
        -- الهدف مقفول وصالح
    else
...
-- ═══ تبديل الهدف باللمس (إصبعان) — للجوال ═══
if UserInputService.TouchEnabled and Config.Aimbot.TouchSwitch ~= false then
    pcall(function()
        ConnectAimbot(UserInputService.TouchTap, function(touchPositions, processedByUI)
            ...
            if touchPositions and #touchPositions >= 2 then
                Aimbot:ForceSwitch()
            end
        end)
    end)
...
-- تشغيل
-- ═══════════════════════════════════════════
RunService:BindToRenderStep("DeltaESP", 200, function()
    safeCall(ESP.Update, ESP)
end)
...
print("[Delta] Mobile Pro loaded.")

-- ═══════════════════════════════════════════
-- القائمة (WindUI) — بدون زر عائم
-- ═══════════════════════════════════════════
safeCall(function()
    ...
    ESPStyle:Slider({
        Title = "Box Thickness", Step = 0.1,
        Value = { Min = 0.5, Max = 4, Default = Config.ESP.BoxThickness },
        Callback = function(v) Config.ESP.BoxThickness = v; ESP:RefreshStyles() end,
    })
    ESPStyle:Slider({
        Title = "Corner Thickness", Step = 0.1,
        Value = { Min = 0.5, Max = 5, Default = Config.ESP.CornerThickness },
        Callback = function(v) Config.ESP.CornerThickness = v; ESP:RefreshStyles() end,
    })
    ESPStyle:Slider({
        Title = "Tracer Thickness", Step = 0.1,
        Value = { Min = 0.5, Max = 4, Default = Config.ESP.TracerThickness },
        Callback = function(v) Config.ESP.TracerThickness = v; ESP:RefreshStyles() end,
    })
    ESPStyle:Slider({
        Title = "Name Size", Step = 1,
        Value = { Min = 8, Max = 24, Default = Config.ESP.NameSize },
        Callback = function(v) Config.ESP.NameSize = v; ESP:RefreshStyles() end,
    })
    ESPStyle:Slider({
        Title = "Health Bar Width", Step = 0.2,
        Value = { Min = 1, Max = 6, Default = Config.ESP.HealthBarWidth },
        Callback = function(v) Config.ESP.HealthBarWidth = v end,
    })

    local ESPPerf = ESPTab:Section({ Title = "Performance", Box = true, BoxBorder = true, Opened = false })
    ESPPerf:Slider({
        Title = "Max Distance", Step = 100,
        Value = { Min = 500, Max = 5000, Default = Config.ESP.MaxDistance },
        Callback = function(v) Config.ESP.MaxDistance = v end,
    })
    ESPPerf:Slider({
        Title = "Mobile FPS", Step = 5,
        Value = { Min = 15, Max = 60, Default = Config.ESP.MobileFPS },
        Callback = function(v) Config.ESP.MobileFPS = v end,
    })
    ESPPerf:Slider({
        Title = "Max Players", Step = 1,
        Value = { Min = 1, Max = 30, Default = Config.ESP.MaxDrawPlayers },
        Callback = function(v) Config.ESP.MaxDrawPlayers = v end,
    })
    ...
    ProtSec:Toggle({
        Title = "Anti Freeze", Value = Config.Protection.AntiFreeze,
        Callback = function(v)
            Config.Protection.AntiFreeze = v
            if v and LocalPlayer.Character then ApplyAntiFreeze(LocalPlayer.Character) end
        end,
    })
    ProtSec:Toggle({
        Title = "Anti Screen Shake", Value = Config.Protection.AntiScreenShake,
        Callback = function(v)
            Config.Protection.AntiScreenShake = v
            if v and Camera then Camera.CameraType = Enum.CameraType.Custom end
        end,
    })
    ...
    SetSec:Keybind({
        Title = "Toggle Menu Key", Value = "RightShift",
        Callback = function(keyName)
    ...
    WindUI:Notify({
        Title = "Delta Hub",
        Content = "افتح القائمة من شريط Delta Hub (أعلى يمين)",
        Icon = "check",
        Duration = 5,
    })
end)
